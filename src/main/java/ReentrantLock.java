...
public class ReentrantLock implements Lock, java.io.Serializable {
    ...
    /** Synchronizer providing all implementation mechanics   同步器 */
    ...
    static final class NonfairSync extends Sync {
        ...
        final boolean initialTryLock() {
            Thread current = Thread.currentThread();
            // CAS修改同步状态state的值
            // 如果成功将state的值从0修改为1，表示
            if (compareAndSetState(0, 1)) { // first attempt is unguarded 第一次尝试是毫无防备的
                setExclusiveOwnerThread(current);
                return true;
            } else if (getExclusiveOwnerThread() == current) {
                int c = getState() + 1;
        ...
        protected final boolean tryAcquire(int acquires) {
            ...
            if (getState() == 0 && compareAndSetState(0, acquires)) {
                setExclusiveOwnerThread(Thread.currentThread());//设置独占模式同步的当前所有者。
                return true;
            }
            return false;
        }
    ...
    static final class FairSync extends Sync {
        ...
        final boolean initialTryLock() {
            ...
            int c = getState();
            if (c == 0) {
                // 不能直接去获取锁（CAS）,要先看队列中是否有线程等待，如果有，则需要乖乖排队
     ...
     * 如果当前线程已经持有锁，则保持计数加1，方法立即返回。
     *
     * <p>If the lock is held by another thread then the
     * current thread becomes disabled for thread scheduling
     * purposes and lies dormant until the lock has been acquired,
     * at which time the lock hold count is set to one.
     * 如果锁被另一个线程持有，那么当前线程出于线程调度的目的将被禁用，
     ...
     * 对外提供的尝试获取锁，获取到返回true,否则返回false
     */
     ...
     * 对外提供的指定时间尝试获取锁，获取到返回true,否则返回false
     */
